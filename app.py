...
def compress_image():
    try:
        ...
        if not file or not file.filename or not isinstance(file.filename, str) or file.filename.strip() == '':
            return jsonify({'error': 'Tidak ada file yang dipilih'}), 400

        # Validasi file gambar
        filename = file.filename if file and file.filename else 'file'
        ...
        return send_file(
            ...
            mimetype='image/jpeg',
            as_attachment=True,
            download_name='compressed_' + (filename or 'file')
        )
...
def convert_image():
    try:
        ...
        filename_out = os.path.splitext(filename)[0] + '.' + output_format.lower()
